...
void on_balance_history(executor *self, Message *msg, local_id from) {
    ...
    debug_worker_print(
        debug_worker_on_hist_fmt, get_lamport_time(), self->local_id, from, history.s_history_len
    );
...
void fill_trailling_history(AllHistory *all_history) {
    ...
    for (int acc_i = 0; acc_i < all_history->s_history_len; ++acc_i) {
        ...
        for (timestamp_t history_t = last_history_t + 1; history_t < acc_history->s_history_len;
             ++history_t) {
            BalanceState state = {
