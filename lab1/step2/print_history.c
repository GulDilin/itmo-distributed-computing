...
void print_history(const AllHistory* history) {
    ...
    for (int i = 0; i < history->s_history_len; ++i) {
        for (int j = 0; j < history->s_history[i].s_history_len; ++j) {
            ...
            if (change->s_balance_pending_in > 0) { has_pending = 1; }
        }
    }

    if (max_time > MAX_T) {
        fprintf(
            stderr, "print_history: max value of s_time: %d, expected s_time < %d!\n", max_time,
            MAX_T
        );
        return;
    }
    ...
    for (int i = 1; i <= history->s_history_len; ++i) {
        for (int j = 0; j <= max_time; ++j) {
            ...
            if (max_cell_width < width) { max_cell_width = width; }
        }
    }
    ...
    for (int i = 0; i < underscrores; ++i) { hline[i] = '-'; }
    hline[underscrores] = '\n';
    ...
    for (int j = 0; j <= max_time; ++j) { printf("%*d |", max_cell_width - 1, j); }
    printf("\n");
    printf("%s", hline);
